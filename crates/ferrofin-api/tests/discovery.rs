//! Discovery handler tests: suggestions, search hints, trailers, movie
...
use ferrofin_model::dto::{BaseItemDto, RecommendationDto, RecommendationType};
use ferrofin_model::entities::MediaStreamType;
use ferrofin_model::querying::{QueryFiltersLegacy, QueryResult};
use ferrofin_model::search::{SearchHint, SearchHintResult, SearchQuery};
...
/// A [`LibraryManager`] backing the discovery routes; unused methods panic.
struct StubLibrary;

#[async_trait]
impl LibraryManager for StubLibrary {
    async fn get_item_by_id(&self, id: Uuid) -> Result<Option<BaseItemEntity>, ServiceError> {
        // The instant-mix seed and the similar-items subject resolve; anything
        // else is absent (→ 404).
        if id == SEED_ID {
            Ok(Some(item_entity(SEED_ID, "Seed")))
        } else if id == ITEM_ID {
            Ok(Some(item_entity(ITEM_ID, "The Item")))
        } else {
            Ok(None)
        }
    }
...
/// A [`SimilarItemsManager`] returning two similar items and one recommendation
/// category.
struct StubSimilar;
...
/// Builds an [`AppState`] wired with the discovery stubs.
fn state() -> AppState {
    AppState::new(
        Arc::new(StubLibrary),
        Arc::new(OkUsers),
...
async fn similar_items_returns_two() {
    for kind in ["Albums", "Artists", "Items", "Movies", "Trailers"] {
        let (status, body) = get(&format!("/{kind}/{ITEM_ID}/Similar")).await;
