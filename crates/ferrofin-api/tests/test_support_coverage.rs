...
use ferrofin_traits::options::{
    DeleteOptions, DtoOptions, InternalItemsQuery, InternalPeopleQuery,
};
...
fn fake_library_methods_panic() {
    ...
    assert_panics(f.get_latest_item_list(q, ferrofin_model::data::CollectionType::movies));
...
fn fake_users_methods_panic() {
    ...
    assert_panics(f.update_policy(Uuid::nil(), &ferrofin_model::users::UserPolicy::default()));
...
fn fake_media_sources_methods_panic() {
    ...
    assert_panics(f.open_live_stream(&ferrofin_model::media_info::LiveStreamRequest::default()));
...
fn fake_sessions_methods_panic() {
    ...
    assert_panics(f.on_playback_start(&ferrofin_model::session::PlaybackStartInfo::default()));
    assert_panics(f.on_playback_progress(
        &ferrofin_model::session::PlaybackProgressInfo::default(),
        false,
    ));
    assert_panics(f.on_playback_stopped(&ferrofin_model::session::PlaybackStopInfo::default()));
    ...
    assert_panics(f.send_play_command("c", "s", &ferrofin_model::session::PlayRequest::default()));
    ...
    assert_panics(f.send_message_to_admin_sessions(
        ferrofin_model::session::SessionMessageType::ForceKeepAlive,
        "d",
    ));
    ...
    assert_eq!(
        ...
        ferrofin_api::test_support::FAKE_ACCESS_TOKEN
    );
    let direct = rt
    ...
    assert_panics(
        f.report_capabilities("s", &ferrofin_model::session::ClientCapabilities::default()),
    );
    assert_panics(
        f.report_transcoding_info("d", &ferrofin_model::session::TranscodingInfo::default()),
    );
...
fn general_command() -> ferrofin_model::session::GeneralCommand {
    ferrofin_model::session::GeneralCommand {
        name: ferrofin_model::session::GeneralCommandType::MoveUp,
